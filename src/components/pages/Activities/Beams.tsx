...
function extendMaterial(BaseMaterial: any, cfg: any) {
  if (typeof window === 'undefined' || !THREE.ShaderLib || !THREE.ShaderLib.physical) {
    return null;
  }
...
const Beams = ({
  ...
  const beamMaterial = useMemo(() => {
    if (typeof window === 'undefined' || !THREE || !THREE.ShaderLib) {
      return null;
    }
    try {
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
      ...
      console.error('Beams material creation error:', err);
      return null;
    }
  }, [speed, noiseIntensity, scale]);

  if (!mounted || typeof window === 'undefined' || error || !beamMaterial) {
    return <div className="beams-container bg-black" style={{ width: '100%', height: '100%' }} />;
  }
  ...
  try {
    ...
    console.error('Beams render error:', err);
    setError(true);
    return <div className="beams-container bg-black" style={{ width: '100%', height: '100%' }} />;
  }
};
