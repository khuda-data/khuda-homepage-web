...
export default function MetallicPaint({
  ...
  const initGL = useCallback(() => {
    ...
    if (!canvas) {
      console.error('MetallicPaint: Canvas not found');
      return false;
    }

    const gl = canvas.getContext('webgl2', { antialias: true, alpha: true });
    if (!gl) {
      console.error('MetallicPaint: WebGL2 is required but not supported. This component requires WebGL2.');
      return false;
    }
  ...
  const uploadTexture = useCallback(imgData => {
    ...
    if (!gl || !imgData) {
      console.error('MetallicPaint: Cannot upload texture - gl or imgData is null');
      return;
    }
    ...
    if (!tex) {
      console.error('MetallicPaint: Failed to create texture');
      return;
    }
    ...
    if (uniforms.u_imgRatio) {
      gl.uniform1f(uniforms.u_imgRatio, ratio);
    }
    
    // canvas 크기에 맞춰 ratio 업데이트
    const canvas = canvasRef.current;
    if (canvas && uniforms.u_ratio) {
  ...
  useEffect(() => {
    if (!initGL()) {
      console.error('MetallicPaint: Failed to initialize WebGL');
      return;
    }
    ...
    const updateCanvasSize = () => {
      ...
      if (rect.width === 0 || rect.height === 0) {
        // Canvas가 아직 렌더링되지 않았을 수 있음
        if (retryCount < maxRetries) {
          retryCount++;
          requestAnimationFrame(updateCanvasSize);
        } else {
          console.warn('MetallicPaint: Canvas size is still 0 after retries');
        }
        return;
      }
      retryCount = 0;
      ...
      if (canvas.width !== width || canvas.height !== height) {
        ...
        if (uniformsRef.current.u_ratio) {
          gl.uniform1f(uniformsRef.current.u_ratio, ratio);
        }
      }
    };

    // 초기 크기 설정 - 약간의 지연을 두어 DOM이 완전히 렌더링된 후 실행
    setTimeout(() => {
      updateCanvasSize();
    }, 0);
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        ...
        console.error('MetallicPaint: Error processing image', error);
      }
    };
    img.onerror = (error) => {
      console.error('MetallicPaint: Failed to load image', imageSrc, error);
      setTextureReady(false);
    };
    img.src = imageSrc;
  ...
  useEffect(() => {
    ...
    if (!gl || !u || !canvas) {
      console.error('MetallicPaint: Missing required refs for rendering');
      return;
    }
